...
export class SyncService implements OnApplicationBootstrap {
    ...
    async handleSync() {
        ...
        try {
            let syncState = await this.prisma.syncState.findUnique({ where: { id: 1 } });
            if (!syncState) {
                syncState = await this.prisma.syncState.create({
                    data: { id: 1, lastOffset: null }
                });
            }
            ...
            while (pagesProcessed < MAX_PAGES_PER_RUN) {
                ...
                const { data, nextPageOffset } = await this.prozorroApi.getTendersPage(currentOffset || undefined);
                ...
                this.logger.log(`Page ${pagesProcessed + 1}: Found ${data.length} tenders, offset: ${currentOffset || 'START'}`);

                // 3. Add valid tenders to queue with retries
                for (const tender of data) {
                    await this.tenderQueue.add('process-tender', { tenderId: tender.id }, {
            ...
            this.logger.log(`Sync cycle finished. Processed ${pagesProcessed} pages.`);
