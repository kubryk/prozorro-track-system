...
export class SearchController {
    ...
    @ApiQuery({ name: 'role', required: false, enum: ['customer', 'supplier'], description: 'Role of identifier: customer (замовник) or supplier (постачальник). Default: customer' })
    ...
    @ApiQuery({ name: 'edrpou', required: false, type: String, description: 'Entity identifier (8 or 10 digits)' })
    @ApiQuery({ name: 'role', required: false, enum: ['customer', 'supplier'], description: 'Role of identifier: customer (замовник) or supplier (постачальник). Default: supplier' })
